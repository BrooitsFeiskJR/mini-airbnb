...
describe("Booking Service", () => {
  ...
  it("should throw an error when trying to make a reservation for a period already reservated", async () => {
    const mockProperty = {
      getId: jest.fn().mockReturnValue(1),
      isAvailable: jest.fn().mockReturnValue(true),
      validateGuestCount: jest.fn(),
    ...
    mockProperty.isAvailable.mockReturnValue(mockUser);
    mockProperty.addBooking.mockImplementationOnce(() => {
      throw new Error("The property is not available for the selected period");
    });
    await expect(bookingService.createBooking(bookingDTO)).rejects.toThrow("The property is not available for the selected period")
  });
});
